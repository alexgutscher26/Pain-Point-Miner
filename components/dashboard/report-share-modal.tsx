...
export function ReportShareModal({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[95vw] overflow-hidden border border-zinc-200 bg-white p-6 text-zinc-950 sm:max-w-lg">
        <DialogHeader>
          <div className="flex items-center gap-2">
            <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-orange-100 text-[#ff4500]">
          ...
          <div>
            ...
            <div className="mt-1.5 flex min-w-0 items-center gap-2">
              <input
                ...
                readOnly
                value={shareUrl}
                className="min-w-0 flex-1 truncate rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2 font-mono text-xs text-zinc-700 focus:outline-none"
              />
              <button
                type="button"
                onClick={handleCopyLink}
                className="flex shrink-0 items-center gap-1 rounded-xl bg-zinc-900 px-3.5 py-2 text-xs font-bold text-white transition-colors hover:bg-zinc-700"
              >
                {copiedLink ? (
          ...
          <div className="border-zinc-150 border-t pt-4">
            <div className="flex items-center justify-between gap-2">
              <div className="flex min-w-0 items-center gap-1.5">
                <Code2 className="h-4 w-4 shrink-0 text-[#ff4500]" />
                <span className="truncate text-xs font-bold text-zinc-900">
                  &quot;Validated with ThreddIQ&quot; Embed Badge
                </span>
              </div>
              <div className="flex shrink-0 items-center rounded-lg border border-zinc-200 bg-zinc-100 p-0.5 text-[10px] font-bold">
            ...
            {/* Code Snippet Box */}
            <div className="relative mt-3 max-w-full overflow-hidden rounded-xl border border-zinc-200 bg-zinc-900 p-3.5 text-white">
              <pre className="max-w-full overflow-x-auto pr-24 font-mono text-[11px] leading-relaxed break-all whitespace-pre-wrap text-zinc-300">
                <code>{embedCodes[embedFormat]}</code>
              </pre>
              <button
                type="button"
                onClick={handleCopyEmbed}
                className="absolute top-2.5 right-2.5 flex items-center gap-1 rounded-md bg-white/10 px-2.5 py-1 text-[10px] font-bold text-white transition-colors hover:bg-white/20"
              >
                {copiedEmbed ? (
