...
export function ComparisonView({
  ...
  const handleSelectA = (id: string) => {
    setSelectedIdA(id);
    router.replace(`/dashboard/compare?a=${id}&b=${selectedIdB}`, {
      scroll: false,
    });
  };
  ...
  const handleSelectB = (id: string) => {
    setSelectedIdB(id);
    router.replace(`/dashboard/compare?a=${selectedIdA}&b=${id}`, {
      scroll: false,
    });
  };
  ...
  const handleExportCSV = () => {
    ...
    const rows = [
      [
        "Metric",
        comparison.reportA.keyword,
        comparison.reportB.keyword,
        "Delta (B vs A)",
      ],
      [
        "Opportunity Score",
        comparison.reportA.opportunityScore,
        comparison.reportB.opportunityScore,
        `${comparison.deltas.opportunityScore.percentChange}%`,
      ],
      [
        "Pain Points Found",
        comparison.reportA.painPointsCount,
        comparison.reportB.painPointsCount,
        `${comparison.deltas.painPointsCount.percentChange}%`,
      ],
      [
        "Avg Urgency (1-10)",
        comparison.reportA.avgUrgency,
        comparison.reportB.avgUrgency,
        `${comparison.deltas.avgUrgency.percentChange}%`,
      ],
      [
        "Avg Monetization (1-10)",
        comparison.reportA.avgMonetization,
        comparison.reportB.avgMonetization,
        `${comparison.deltas.avgMonetization.percentChange}%`,
      ],
      [
        "Blue Ocean Niches",
        comparison.reportA.quadrantBreakdown.blueOcean,
        comparison.reportB.quadrantBreakdown.blueOcean,
        `${comparison.deltas.blueOceanCount.percentChange}%`,
      ],
      [
        "Budget Signals",
        comparison.reportA.budgetSignalsCount,
        comparison.reportB.budgetSignalsCount,
        `${comparison.deltas.budgetSignalsCount.percentChange}%`,
      ],
    ];
    const csvContent =
    ...
    link.setAttribute(
      "download",
      `comparison-${comparison.reportA.keyword}-vs-${comparison.reportB.keyword}.csv`,
    );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col items-stretch justify-between gap-4 rounded-2xl border border-zinc-200/80 bg-white p-4 shadow-xs sm:p-5 lg:flex-row lg:items-center dark:border-zinc-800 dark:bg-zinc-900/70">
        <div className="grid flex-1 grid-cols-1 gap-4 sm:grid-cols-2">
          {/* Selector A */}
          <div className="space-y-1.5">
            <label className="flex items-center gap-1.5 font-mono text-[10px] font-bold tracking-wider text-zinc-500 uppercase dark:text-zinc-400">
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-xl border border-zinc-200 bg-zinc-50/80 px-3.5 py-2.5 pr-8 text-xs font-bold text-zinc-900 transition-all outline-none hover:border-black/20 focus:border-[#ff4500] focus:bg-white dark:border-zinc-800 dark:bg-zinc-950 dark:text-white"
              >
                {availableReports.map((r) => (
                  <option
                    key={r.id}
                    value={r.id}
                    disabled={r.id === selectedIdB}
                  >
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              <select
                ...
                className="w-full appearance-none rounded-xl border border-zinc-200 bg-zinc-50/80 px-3.5 py-2.5 pr-8 text-xs font-bold text-zinc-900 transition-all outline-none hover:border-black/20 focus:border-blue-600 focus:bg-white dark:border-zinc-800 dark:bg-zinc-950 dark:text-white"
              >
                {availableReports.map((r) => (
                  <option
                    key={r.id}
                    value={r.id}
                    disabled={r.id === selectedIdA}
                  >
                    {r.keyword} ({r.painPointsCount} pts, Score: {r.score})
                  </option>
                ))}
        ...
        {/* Export Buttons */}
        {comparison && (
          <div className="flex shrink-0 items-center gap-2 border-t border-zinc-100 pt-2 lg:border-t-0 lg:pt-0 dark:border-zinc-800">
            <button
              onClick={handleExportCSV}
              className="inline-flex cursor-pointer items-center gap-1.5 rounded-xl border border-zinc-200 bg-white px-3.5 py-2 font-mono text-[11px] font-bold text-zinc-700 uppercase transition-all hover:border-[#ff4500] hover:text-[#ff4500] dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-300 dark:hover:text-[#ff4500]"
            >
      ...
      {!comparison ? (
        <div className="rounded-2xl border border-zinc-200/80 bg-white p-12 text-center shadow-xs dark:border-zinc-800 dark:bg-zinc-900/70">
          ...
          <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
            Choose two distinct research dossiers from the dropdown menus above
            to analyze pain point densities and market opportunities
            side-by-side.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="relative overflow-hidden rounded-2xl border border-[#ff4500]/30 bg-gradient-to-br from-[#ff4500]/10 via-[#ff4500]/5 to-transparent p-6 shadow-sm dark:from-[#ff4500]/15 dark:via-[#ff4500]/5">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-2 rounded-md bg-[#ff4500]/20 px-2.5 py-1 font-mono text-[10px] font-black tracking-widest text-[#ff4500] uppercase">
                ...
                <p className="max-w-3xl text-sm font-medium text-zinc-600 dark:text-zinc-300">
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <div className="space-y-4 rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-xs dark:border-zinc-800 dark:bg-zinc-900/70">
              <div className="flex items-center justify-between border-b border-zinc-100 pb-3 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                  <Compass className="h-4 w-4 text-[#ff4500]" />
                  <h4 className="font-mono text-sm font-bold tracking-wider text-zinc-950 uppercase dark:text-white">
          ...
          <div className="space-y-6 rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-xs dark:border-zinc-800 dark:bg-zinc-900/70">
            <div className="flex items-center justify-between border-b border-zinc-100 pb-3 dark:border-zinc-800">
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-2 px-1">
                  ...
                  <h5 className="truncate font-mono text-xs font-black text-zinc-900 uppercase dark:text-white">
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 space-y-1">
                            ...
                            <p className="line-clamp-2 text-xs font-bold text-zinc-900 dark:text-white">
                              {item.painPointA.title}
                            </p>
                          </div>
                          <span className="shrink-0 rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-2 py-0.5 font-mono text-[11px] font-black text-emerald-600 dark:text-emerald-400">
                  ...
                  {comparison.topPainPointsHeadToHead.map((item) =>
                    item.painPointB ? (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0 space-y-1">
                            <span className="font-mono text-[10px] font-bold text-blue-600 uppercase dark:text-blue-400">
                              #{item.rank} Friction Signal
                            </span>
                            <p className="line-clamp-2 text-xs font-bold text-zinc-900 dark:text-white">
...
function DeltaMetricCard({
  ...
  return (
    <div className="flex flex-col justify-between rounded-2xl border border-zinc-200/80 bg-white p-5 shadow-xs dark:border-zinc-800 dark:bg-zinc-900/70">
      <div>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3 border-b border-zinc-100 pb-3 dark:border-zinc-800">
          <div>
            <span className="block truncate font-mono text-[9px] font-bold text-[#ff4500] uppercase">
              {labelA}
            </span>
          ...
          <div>
            <span className="block truncate font-mono text-[9px] font-bold text-blue-600 uppercase dark:text-blue-400">
              {labelB}
            </span>
            <span className="text-xl font-black text-zinc-900 dark:text-white">
              {valueB}
      ...
      <div className="mt-3 flex items-center justify-between font-mono text-xs">
        <span className="text-[10px] text-zinc-400 uppercase">Delta Diff</span>
        <span
...
function QuadrantComparisonRow({
  ...
  return (
    <div
      className={`rounded-xl p-3 ${highlight ? "border border-[#ff4500]/20 bg-[#ff4500]/5" : "bg-zinc-50/50 dark:bg-zinc-950/40"}`}
    >
      <div className="mb-2 flex items-center justify-between text-xs font-bold text-zinc-900 dark:text-white">
        <span>{name}</span>
      ...
      <div className="space-y-1.5">
        <div className="flex items-center gap-2">
          <span className="w-5 font-mono text-[9px] font-bold text-[#ff4500]">
            A
          </span>
        ...
        <div className="flex items-center gap-2">
          <span className="w-5 font-mono text-[9px] font-bold text-blue-600 dark:text-blue-400">
            B
          </span>
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-zinc-200 dark:bg-zinc-800">
            <div
...
function SentimentComparisonRow({
  ...
  return (
    <div className="rounded-xl bg-zinc-50/50 p-3 dark:bg-zinc-950/40">
      <div className="mb-2 flex items-center justify-between text-xs font-bold text-zinc-900 dark:text-white">
        <span>{emotion}</span>
        <span className="font-mono text-[11px] text-zinc-500">
          A: {countA} ({pctA}%) vs B: {countB} ({pctB}%)
