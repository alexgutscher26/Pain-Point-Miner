...
export async function POST(req: Request) {
  try {
    ...
    if (pack === "starter") {
      creditsToAdd = 50;
      if (currentUser?.ltdTier === "professional")
        priceId = process.env.STRIPE_PRICE_CREDITS_50_PRO!;
      else if (currentUser?.ltdTier === "founder")
        priceId = process.env.STRIPE_PRICE_CREDITS_50_FOUNDER!;
      else priceId = process.env.STRIPE_PRICE_CREDITS_50!;
    } else if (pack === "pro") {
      creditsToAdd = 250;
      if (currentUser?.ltdTier === "professional")
        priceId = process.env.STRIPE_PRICE_CREDITS_250_PRO!;
      else if (currentUser?.ltdTier === "founder")
        priceId = process.env.STRIPE_PRICE_CREDITS_250_FOUNDER!;
      else priceId = process.env.STRIPE_PRICE_CREDITS_250!;
    } else if (pack === "elite") {
      creditsToAdd = 1000;
      if (currentUser?.ltdTier === "professional")
        priceId = process.env.STRIPE_PRICE_CREDITS_1000_PRO!;
      else if (currentUser?.ltdTier === "founder")
        priceId = process.env.STRIPE_PRICE_CREDITS_1000_FOUNDER!;
