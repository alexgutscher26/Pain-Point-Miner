...
export const metadata: Metadata = {
  title: "Reading a Desperation Score: What Actually Makes a Complaint 'Validated' | ThreddIQ Blog",
  description: "Behind the scenes of ThreddIQ's scoring engine — how we separate validated pain points from noise using desperation, frequency, budget signals, and competitive context.",
};
...
const scoreRanges = [
  { range: "1–3", label: "Casual noise", desc: "Mild frustration or hypothetical wondering. Not actionable.", color: "bg-zinc-500/10 text-zinc-600" },
  { range: "4–6", label: "Signal emerging", desc: "Real frustration but low urgency. Worth monitoring. May grow.", color: "bg-amber-500/10 text-amber-700" },
  { range: "7–8", label: "Validated pain point", desc: "Clear frustration + workaround or tool mention. Act on this.", color: "bg-[#ff4500]/10 text-[#ff4500]" },
  { range: "9–10", label: "Hot buying signal", desc: "Desperation + budget + competitor mention. Drop everything.", color: "bg-green-500/10 text-green-700" },
];
      ...
      <main className="mx-auto flex w-full max-w-5xl flex-col px-6 pt-32 pb-24">
        ...
        <header className="mb-20">
          ...
          <h1 className="mb-8 text-[40px] leading-tight font-black tracking-tight text-zinc-900 md:text-[72px]">
            Reading a{" "}
            <span className="text-[#ff4500]">desperation score</span>
          </h1>
          ...
          <p className="max-w-3xl text-xl leading-relaxed font-medium text-zinc-500 md:text-2xl">
            Every Reddit post ThreddIQ scans gets a desperation score from 1 to 10. It&apos;s the
            single most important number in our system — and once you understand how it works,
            you&apos;ll never look at a complaint the same way again.
          </p>
        </header>

        <section className="mb-32">
          <h2 className="mb-10 text-3xl font-black text-zinc-900 md:text-4xl">The five factors behind every score</h2>
          <p className="mb-12 text-lg font-medium text-zinc-500 leading-relaxed md:text-xl">
            No single factor determines the score. It&apos;s a weighted composite of five signals, each
            contributing to the final number:
          </p>
          <div className="space-y-10">
            {factors.map((f, i) => (
              <div key={i} className="flex flex-col gap-4 md:flex-row md:items-start md:gap-8">
                <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl border border-[#ff4500]/20 bg-[#ff4500]/5">{f.icon}</div>
                <div className="flex-1">
                  <div className="mb-2 flex items-center gap-3">
                    <h3 className="text-xl font-black text-zinc-900">{f.title}</h3>
                    <span className="rounded-full bg-zinc-500/10 px-3 py-0.5 text-[11px] font-black text-zinc-600 uppercase tracking-widest">{f.weight}</span>
                  </div>
                  <p className="text-lg font-medium text-zinc-500 leading-relaxed">{f.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-32 rounded-[48px] glass-card p-12 md:p-20">
          <h2 className="mb-8 text-3xl font-black text-zinc-900 md:text-4xl">Score ranges: what each tier means for you</h2>
          <div className="space-y-4">
            {scoreRanges.map((r, i) => (
              <div key={i} className="flex items-baseline gap-6 rounded-[16px] border border-black/5 p-6">
                <span className="text-3xl font-black text-zinc-900 min-w-[60px]">{r.range}</span>
                <div>
                  <span className={`inline-block rounded-full px-3 py-0.5 text-[10px] font-black uppercase tracking-widest ${r.color}`}>{r.label}</span>
                  <p className="mt-2 text-[15px] font-medium text-zinc-500">{r.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-32">
          <h2 className="mb-10 text-3xl font-black text-zinc-900 md:text-4xl">See it in action</h2>
          <div className="space-y-8">
            {examples.map((ex, i) => (
              <div key={i} className="rounded-[24px] glass-card p-8">
                <div className="mb-4 flex items-center gap-4">
                  <span className="text-4xl font-black text-[#ff4500]">{ex.score}</span>
                  <span className="text-xs font-black uppercase tracking-widest text-zinc-500">Desperation Score</span>
                </div>
                <p className="mb-4 text-lg font-bold text-zinc-800 italic leading-relaxed">{ex.text}</p>
                <div className="rounded-[16px] border border-black/5 bg-black/2 p-5">
                  <p className="text-sm font-medium text-zinc-500 leading-relaxed"><span className="font-bold text-zinc-700">Why this score:</span> {ex.why}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-32 border-l-4 border-[#ff4500] glass-card py-12 px-10 rounded-r-[32px]">
          <blockquote className="text-3xl font-black italic leading-tight text-zinc-900 md:text-4xl">
            &ldquo;The desperation score is the single most useful signal we track. It cuts through the
            noise and tells us exactly which threads to drop everything for.&rdquo;
          </blockquote>
        </section>
        ...
        <section className="mb-32 border-t border-black/10 py-16 text-center">
          <p className="mx-auto max-w-2xl text-xl font-medium text-zinc-500 italic md:text-2xl leading-relaxed">
            The desperation score turns Reddit from a firehose of opinions into a ranked list of
            market opportunities. Every post gets a number. Every number tells you what to do next.
          </p>
        </section>
        ...
        <div className="relative mt-24 flex flex-col items-center overflow-hidden rounded-[48px] glass-card p-16">
          ...
          <h2 className="mb-6 text-center text-3xl font-extrabold text-zinc-900 md:text-5xl">
            See every post&apos;s{" "}
            <span className="text-[#ff4500]">score</span>
          </h2>
          <p className="mb-12 max-w-2xl text-center text-xl leading-relaxed font-medium text-zinc-500">
            ThreddIQ scores every Reddit pain point across your niche. Stop guessing which
            complaints matter. Start building what people will actually pay for.
          </p>
          <div className="flex flex-col gap-4 sm:flex-row">
            <Button asChild size="lg" className="rounded-2xl bg-linear-to-b from-[#ff5100] to-[#e63e00] px-12 py-7 text-xl font-black text-white shadow-xl shadow-[#ff4500]/20 transition-all hover:from-[#ff621a] hover:to-[#ff4500]">
              <Link href="/sign-up">Try the Score Engine <ArrowRight className="ml-2 h-5 w-5" /></Link>
            </Button>
          </div>
        </div>
      </main>
