...
const KEYWORDS = Array.from(
  { length: CONCURRENT_RUNS },
  (_, i) => `loadtest-${i}-${Date.now()}`,
);

async function runSimulation() {
  console.log(
    `🚀 Starting Load Test: ${CONCURRENT_RUNS} concurrent mining runs...`,
  );
  ...
  const tasks = KEYWORDS.map(async (keyword, idx) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        console.error(
          `❌ Request ${idx} failed (${res.status}): ${text.slice(0, 100)}`,
        );
        results.failed++;
        return;
      }
      ...
      const streamRes = await fetch(
        `${TARGET_URL}/api/search/stream?id=${scraperId}`,
        {
          headers: {
            "x-load-test-id": USER_ID,
          },
  ...
  console.log(
    `📡 SSE Stability: ${results.streamStability.filter(Boolean).length}/${results.success}`,
  );

  if (results.failed > 0) {
    console.log(
      "⚠️ WARNING: Some requests failed. This may indicate connection pool exhaustion or rate limits.",
    );
  } else {
    console.log(
      "🌟 Baseline established: System handled 50 concurrent hits without panic.",
    );
  }
}
